...
class RootMeter(object):
    """Computes and stores the average and current value return the squareroot"""
...
# Removed Confusion matrix because its not a classification task
...
# accuracy from CSRNet on image to image

def accuracy_image2image(output, target):
    return{
        'mae': abs(output.data.sum()-target.sum()),
        'mse': (abs(output.data.sum()-target.sum()))**2
    }
